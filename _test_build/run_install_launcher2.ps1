...
$tmpJar = Join-Path $w '_test_build\StarlightLauncher-2.0.0.jar'
Copy-Item $jar $tmpJar -Force
Write-Output ('TMP=' + (Get-Item $tmpJar).Length + 'B')
Write-Output '===== mvn install:install-file ====='
& mvn -q install:install-file "-Dfile=$tmpJar" '-DgroupId=com.starlight' '-DartifactId=starlight-launcher-core' '-Dversion=2.0.0' '-Dpackaging=jar' 2>&1 | ForEach-Object { Write-Output $_ }
